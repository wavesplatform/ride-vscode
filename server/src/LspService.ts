...
import { globalSuggestions, globalVariables, transactionClasses, types as fieldsMap, functions as funcsMap, types } from './suggestions/index'
...
export class LspService {
                ...
                case (character === '.' || wordBeforeDot !== null):                 //autocompletion after clicking on a dot
                    ...
                    switch (true) {
                        case (['buyOrder', 'sellOrder'].indexOf(inputWord) > -1):
                            result = fieldsMap['Order'].fields;                            //ExchangeTransaction:'buyOrder', 'sellOrder'
                            break;
                        case (['recipient'].indexOf(inputWord) > -1):               //Transfer:'recipient'
                            result = [...fieldsMap['Address'].fields, ...fieldsMap['Alias'].fields];
                            break;
                        case (['tx'].indexOf(inputWord) > -1):                      // 'tx'
                            result = intersection(...transactionClasses.map(val => fieldsMap[val].fields));
                            break;
    ...
    public hover(document: TextDocument, position: Position) {
        ...
        if (funcsMap[word]){
            result.push(`**${word}** (\n
                ...
                ` ${p.type} - ${p.detail}`} \n`)}\n) : ${funcsMap[word].type} \n>_${funcsMap[word].detail}_`);
        }else if(fieldsMap[word]){
            result.push(fieldsMap[word].detail)
    ...
    public signatureHelp(document: TextDocument, position: Position): SignatureHelp {
        ...
        const functionArguments = textBefore.split(lastFunction.pop()).pop()
        ...
        try {
            ...
            if (funcsMap[word])
                result.push({
                    ...
                    label: `${word}(${funcsMap[word].params.map((p: ParamType) => `${p.name}${p.required ? "" : "?"}:` +
                        ` ${p.type}`).join(', ')}): ${funcsMap[word].type}`,
                    documentation: funcsMap[word].detail,
                    parameters: funcsMap[word].params.map((p: ParamType) => ({
            ...
            out = {
                ...
                activeParameter: (functionArguments.split(',').length - 1),
                activeSignature: 0,
                signatures: result,
            };
        } catch (e) { }
